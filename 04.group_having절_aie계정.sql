/*
    <GROUP BY절>
    그룹기준을 제시할수 있는 구문(여러 그룹 기준별로 여러 그룹으로 묶을수 있음)
    여러개의 값들을 하나의 그룹으로 묶어서 처리할 목적으로 사용
*/
SELECT SUM(SALARY) FROM EMPLOYEE;   --전체 사원을 하나의 그룹으로 무꺼서 총합을 구함
...
--각 직급별 사원수와 급여합계 조회
SELECT JOB_CODE,COUNT(JOB_CODE),SUM(SALARY) FROM EMPLOYEE GROUP BY JOB_CODE ORDER BY JOB_CODE DESC;

--각 직급별 총 사원수,보너스를 받는 사원수,급여합,평균급여,최고급여 조회(직급별 오름차순 정렬)
SELECT JOB_CODE,COUNT(*),COUNT(BONUS),SUM(SALARY),ROUND(AVG(SALARY)),MIN(SALARY),F(SALARY) 
FROM EMPLOYEE 
GROUP BY JOB_CODE 
ORDER BY JOB_CODE;
...
/*
    <HAVING 절>
    그룹에 대한 조건을 제시할때 사용되는 구문(주로 그룹함수식을 가지고 조건을 제시할때 사용_

--각 부서별 평균 급여 조회(부서코드,평균급여)
SELECT DEPT_CODE,CEIL(AVG(SALARY)) FROM EMPLOYEE GROUP BY DEPT_CODE;

--각 부서별 평균 급여가 300만원 이상인 부서만 조회
SELECT DEPT_CODE,CEIL(AVG(SALARY))
FROM EMPLYEE
--WHERE AVG(SALARY) >= 3000000 --오류:그룹함수에선 조건 제시시 WHERE절에서는 안됨
GROUP BY DEPT_CODE;
*/
SELECT DEPT_CODE,CEIL(AVG(SALARY))
FROM EMPLOYEE
...
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(BONUS)=0;  --COUNT가 0인부서

-------------------------------------------------------------------------------------------------
/*
    <집계 함수>
    그룹별 산출된 결과 값에 중간집곌ㄹ 계산해 주는 함수
    
    ROLLUP,CUBE
    =>GROUP BY절에 기술하는 함수
    - ROLLUP(컬럼1,컬럼2):컬럼1을 가지고 다시 중간집계를 내는 함수
    - CUBE(컬럼1,컬럼2):컬럼1을 가지고 다시 중간집계를 내고 컬럼2를 자기고 다시 중간집곌를 내는 함수
*/
--각 직급별 급여합
...
FROM EMPLOYEE
GROUP BY CUBE(DEPT_CODE,JOB_CODE) --NULL : 중간집계
ORDER BY 1;

SELECT DEPT_CODE,JOB_CODE,SUM(SALARY)
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE,JOB_CODE) --NULL : 중간집계 (CUBE보다 ROLLUP이 중간집계용으로 쓴다)
ORDER BY 1;
...
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE='D5' OR SALARY>3000000; --12명(결과값상동)
...
--주의사항
--집합연산자를 사용할경우 컬럼의 갯수와 컬럼명이 동일해야함
-------------------------------3.UNION ALL------------------------------------------------------------------
...
WHERE SALARY>3000000;
-------------------------------4.MINUS------------------------------------------------------------------
--직급코드가 D5인 사원들중 급여가 300만원 초과인 사월을 제외한 사원조회
