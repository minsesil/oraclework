/*
 < 뷰 : VIEW >
 SELECT 문을 저장해 둘수 있는 객체
 (자주 쓰는 긴 SELECT문을 저장해두면 그 긴 SELECT문을 매번 다시 기술할 필요없음)
 임시테이블 같은 존재(실제 데이타가 담겨있는건 아님 => 논리적인 테이블)
 */
...
WHERE NATIONAL_NAME = '일본';
 ...
 -->매번 바뀔수 있는 나라이름인데 나라만 바뀔경우 구문을 다 써야한다. 이럴경우 VIEW 생성한다.
/* 뷰에 넣을 서브쿼리
 SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
 FROM EMPLOYEE
 JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
--관리자계정에서 VIEW를 생성할수 있는 권한을 부여해준다
...
FROM (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
      FROM EMPLOYEE
       ...
       JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
       JOIN NATIONAL USING (NATIONAL_CODE));
--뷰는 논리적인 가상테이블(실질적으로는 데이터를 저장하고 있지않음)
...
-------------------------------------------------------------------------------------
 /*
   뷰 컬럼에 별칭 부여
   서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어 있을경우 반드시 별칭 지정 해야됨.
*/
--전체 사원의 사번, 사원명, 직급명, 성별(남/여), 근무년수를 조회할 수 있는 VIEW 생성
...
CREATE OR REPLACE VIEW VM_EMP_JOB(사번, 사원명, 직급명, 성별,근 무년수)
AS SELECT EMP_ID
              , EMP_NAME
              , JOB_NAME
...
--뷰를 통해 조작하면 실제 데이타가 담겨있는 베이스 테이블에 반영됨
...
/*
  * 단, DML명령어로 조작이 불가능한 경우가 더 많음
  ...
  2)뷰에 정의되어 있지 않은 컬럼 중에 베이스테이블 상에 NOT NULL 제약 조건이 지정되어있는 경우
  3)산술연산식이나 함수식으로 정의되어 있는 경우
  4)그룹함수나 GROUP BY절이 포함되어 있는 경우
  5)DISTINCT 구문이 포함된 경우
  6)JOIN을 이용하여 여러 테이블을 연결시켜 놓은경우
*/
--뷰 정의
CREATE OR REPLACE VIEW VM_JOB
AS SELECT JOB_CODE 
...
--뷰를 통해 INSERT(오류)
INSERT INTO VM_JOB(JOM_CODE, JOB_NAME) VALUES('J8','인턴');
); 
...
--INSERT
INSESRT INTO VM_JOB VALUES('사원');
-->오류 JOB_CODE 가 PRIMARY KEY로 널이오면 안됨

--실제 테이블에 INSERT할때는 VALUES(NULL,'사원')추가
--            JOB_CODE는 PK라서 NULL을 넣을수 없음

--DELETE
DELETE FROM VM_JOB WHERE JOB_NAME='사원';
-->외래키가 되어있는경우->무결성제약조건위배.자식레코드발견. 

--3)산술연산식이나 함수식으로 정의되어 있는경우
CREATE OR REPLACE VIEW VM_EMP_SAL
AS SELECT EMP_ID, EMP_NAME, SALARY, SALARY*12 연봉  --산술식은 반드시 별칭써야함
   FROM EMPLOYEE;

--INSERT(오류):연봉은 기존테이블에 없음 (오류내용:가상 열은 사용할수 없습니다)
INSERT INTO VM_EMP_SAL VALUES(600, '김상진', 3000000, 36000000);

INSERT INTO VM_EMP_SAL VALUES(EMP_ID, EMP_NAME, SALARY) VALUES(600, '김상진', 3000000);
-->오류
...
SELECT * FROM VM_EMP;  --테이블 생성하고 나면 이젠 테이블이 보인다
...
--UPDATE : 200번 사원의 급여를 200만원으로 변경
UPDATE VM_EMP 
   SET SALARY = 2000000 
WHERE EMP_ID=200; --행이 삭제됨

SELECT * FROM VM_EMP; -- 7명조회

--==WITH CHECK OPTION을써서 생성
...
--UPDATE(오류):서브쿼리에 기술된 조건에 부합되지 않기 때문에 변경불가
UPDATE VM_EMP_CHECK SET SALARY = 2000000 WHERE EMP_ID=201;

--202번을 400만원으로 수정(성공)
UPDATE VM_EMP_CHECK SET SALARY = 4000000 WHERE EMP_ID=202;
