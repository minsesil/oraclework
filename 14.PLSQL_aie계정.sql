...
BEGIN
  RESULT := 10/&숫자;
  DBMS_OUPTPUT.PUT_LINE('결과:'||RESULT);
EXCEPTION
...
--UNIQUE 제약조건 위배
BEGIN
  UPDATE EMPLOYEE SET EMP_ID = '&변경할사번';   --사용자로부터받음
  WHERE EMP_NAME = '김정보';
EXCEPTION
...
EXCEPTION
 ...
 --WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('조회된 결과가 너무 많거나 하나도 없습니다');  --위둘을 모르면 OTHERS로 써주어도 된다.
END;
